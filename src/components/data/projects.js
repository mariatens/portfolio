export const ProjectData=[
    {
        id:1,
        title:"TV Shows Project",
        about: "The TV Shows webpage is a project developed in collaboration with a colleague.It fetches data from the TV Maze API and shows it in a compelling way using React and Typescript. It allows the user to choose from different TV shows and filter for specific episodes.",
        tags:["React", "TypeScript", "HTML", "CSS"],
        url:"https://tv-shows-project-alli-maria.netlify.app/",
        github:"https://github.com/mariatens/tv-shows-project",
        image:""
    },
    {
        id:2,
        title:"Paste Bin",
        about: "Paste bin with functionality to comment on pastes and delete pastes/comments",
        tags:["React", "Typescript", "Node", "Express"],
        demo:"https://github.com/ThantZinPhyo-OP/Portfolio-Website",
        github:"https://github.com/DidierOsindero/paste-bin-frontend",
        image:""
    },
    {
        id:3,
        title:"To do app",
        about: "App with functionality to add, delete, edit and mark To Dos as complete",
        tags:["React", "Typescript", "Node", "Express"],
        demo:"https://mariatens-todo.netlify.app/",
        github:"https://github.com/mariatens/todo-front-end",
        image:""   
    }
];